...
def make_decoder(opt, embeddings, stage1, decoder2=False):
    """
    ...
    if stage1:
        ...
        if decoder2:
            return StdRNNDecoder(opt.rnn_type, opt.brnn2,
...
def make_base_model(model_opt, fields, gpu, checkpoint=None, stage1=True):
    ...
    if not stage1:
        src2_dict = fields["src2_all"].vocab
        feature_dicts = []
        src2_embeddings = make_embeddings(model_opt, src2_dict, feature_dicts)
        decoder2 = make_decoder(model_opt, src2_embeddings, stage1, decoder2=True)
    # Make NMTModel(= encoder + decoder).
    # model = NMTModel(encoder, decoder)
    if stage1:
        model = NMTModel(encoder, decoder)
    else:
        model = NMTModel(encoder, decoder, decoder2=decoder2)
    ...
    if stage1:
        ...
        generator2 = nn.Sequential(
            nn.Linear(model_opt.rnn_size, len(fields["src2_all"].vocab)),
            nn.LogSoftmax())
