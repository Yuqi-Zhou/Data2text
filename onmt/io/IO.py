...
def build_dataset(
    ...
    :param tgt_seq_length:
    :param src_seq_length_trunc:
    :param tgt_seq_length_trunc:
    :param dynamic_dict:
    :param sample_rate:
    :param window_size:
    :param window_stride:
...
def _build_field_vocab(field, counter, *args, **kwargs):
    """
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                if val is not None and k in (
                        'indices', 'src_map', 'alignment'):
                    val = [val]
                counter[k].update(val)

    for tgt in ("tgt1", "tgt2", "tgt2_ptrs"):
        _build_field_vocab(fields[tgt], counter[tgt], tgt,
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["src2_all"], counter["src2_all"],
                           max_size=src_vocab_size,
                           min_freq=src_words_min_frequency)
        print(" * src2_all vocab size: %d." % (len(fields["src2_all"].vocab)))
