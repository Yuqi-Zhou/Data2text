...
def load_fields(dataset, data_type, checkpoint, opt):
    ...
    if data_type == 'text' or data_type == 'box':
        print(
            ' * vocabulary size. source1 = %d; target1 = %d, source2 = %d; target2 = %d' %
            (len(
                fields['src1'].vocab), len(
                fields['tgt1'].vocab), len(
                fields['src2'].vocab), len(
                    fields['tgt2'].vocab)))
    """
    else:
        assert False
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    if checkpoint is None:
        model1 = onmt.ModelConstructor.make_base_model(
            model_opt, fields, use_gpu(opt), checkpoint, stage1=True)
        model2 = onmt.ModelConstructor.make_base_model(
            model_opt, fields, use_gpu(opt), checkpoint, stage1=False)
    else:
        model1 = onmt.ModelConstructor.make_base_model(
            model_opt, fields, use_gpu(opt), checkpoint[0], stage1=True)
        model2 = onmt.ModelConstructor.make_base_model(
            model_opt, fields, use_gpu(opt), checkpoint[1], stage1=False)
...
def make_loss_compute(model, tgt_vocab, opt, stage1=True, loss2=False):
    """
    ...
    if not stage1:
        ...
        '''
        if loss2:
            generator = model.generator2
            compute = onmt.Loss.NMTLossCompute(
                generator, tgt_vocab,
                label_smoothing=opt.label_smoothing, decoder_type="lstm")
        else:
...
def train_model(
    ...
    train_loss = make_loss_compute(
        model, fields["tgt1"].vocab, opt, stage1=True)

    valid_loss = make_loss_compute(
        model, fields["tgt1"].vocab, opt, stage1=True)

    train_loss2 = make_loss_compute(
        model2, fields["tgt2"].vocab, opt, stage1=False)

    valid_loss2 = make_loss_compute(
        model2, fields["tgt2"].vocab, opt, stage1=False)
    '''

    if opt.train_from1 and opt.train_from2:
        train_loss3 = make_loss_compute(model2, fields["src2"].vocab, opt, stage1=False, loss2=True)
        valid_loss3 = make_loss_compute(model2, fields["src2"].vocab, opt, stage1=False, loss2=True)
        finetune = True
    else:
        train_loss3 = None
        valid_loss3 = None
        ...
        finetune = True
    '''
    train_loss3 = make_loss_compute(
        model2,
        fields["src2_all"].vocab,
        opt,
        stage1=False,
        loss2=True)
    valid_loss3 = make_loss_compute(
        model2,
        fields["src2_all"].vocab,
        opt,
        stage1=False,
    ...
    '''
    trainer = onmt.Trainer(model, model2, train_loss, valid_loss, train_loss2, valid_loss2,
                           optim, optim2, trunc_size, shard_size, data_type,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if opt.lm_finetune:
            train_stats, train_stats2 = trainer.train(
                train_iter, epoch, opt, report_func)
        elif finetune:
            train_stats, train_stats2, train_stats3 = trainer.train_ft(
                train_iter, epoch, opt, report_func)
        else:
            train_stats, train_stats2 = trainer.train(
                train_iter, epoch, opt,report_func)
        ...
        if finetune and not opt.lm_finetune:
            print('Train perplexity3: %g' % train_stats3.ppl())
            print('Train accuracy3: %g' % train_stats3.accuracy())

        # 2. Validate on the validation set.
        valid_iter = make_dataset_iter(lazily_load_dataset("valid"),fields, opt, is_train=False)
        # valid_stats, valid_stats2, valid_stats3 = trainer.validate(valid_iter)
        # valid_stats, valid_stats2 = trainer.validate(valid_iter)
        if opt.lm_finetune:
            valid_stats, valid_stats2 = trainer.validate(valid_iter)
        elif finetune:
            valid_stats, valid_stats2, valid_stats3 = trainer.validate_v2(
                valid_iter)
        else:
            valid_stats, valid_stats2 = trainer.validate(valid_iter)
        ...
        if finetune and not opt.lm_finetune:
            print('Validation perplexity3: %g' % valid_stats3.ppl())
            print('Validation accuracy3: %g' % valid_stats3.accuracy())
        ...
        if epoch >= opt.start_checkpoint_at:
            print("drop checkpoint...")
            trainer.drop_checkpoint(
                model_opt, epoch, fields, valid_stats, valid_stats2)
...
def train(opt):
    ...
    if opt.train_from1 and opt.train_from2:
        ...
        print('Loading checkpoint from %s\n%s' %
              (opt.train_from1, opt.train_from2))
        checkpoint = []
        checkpoint.append(
            torch.load(
                opt.train_from1,
        ...
        checkpoint.append(
            torch.load(
                ...
                map_location=lambda storage,
                                    loc: storage))
        model_opt = checkpoint[0]['opt']
        model_opt.save_model = opt.save_model

        opt.start_epoch = checkpoint[0]['epoch'] + 1
    else:
        checkpoint = None
        model_opt = opt
    ...
    # checkpoint[0]["vocab"] == checkpoint[1]["vocab"]
    if checkpoint is not None:
        if not opt.train_from1:
            fields = load_fields(first_dataset, 'text', checkpoint[0], opt)
        else:
            # author model doesn't have src2_all vocab
            fields = load_fields(first_dataset, 'text', None, opt)
    else:
        fields = load_fields(first_dataset, 'text', checkpoint, opt)
    ...
    # Build optimizer.
    optim1 = build_optim(model1, checkpoint, opt)
    optim2 = build_optim(model2, checkpoint, opt)
