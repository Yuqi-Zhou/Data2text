        ...
        if sent[i] in prons:
            if resolve_prons:
                ...
                if referent is None:
                    ...
                    # print "replacing", sent[i], "with", referent[2], "in", "
                    # ".join(sent)
...
def annoying_number_word(sent, i):
    ...
    ignores = {"three point", "three - point", "three - pt", "three pt", "three - pointers", "three - pointer",
               "three pointers"}
    return " ".join(
        sent[i:i + 3]) in ignores or " ".join(sent[i:i + 2]) in ignores
...
def extract_numbers(sent):
    ...
    while i < len(sent):
        ...
        # get longest span  (this is kind of stupid)
            ...
            j = 1
            while i + \
                    j < len(sent) and sent[i + j] in number_words and not annoying_number_word(sent, i + j):
                j += 1
            try:
    ...
    if len(keys) == 0:
        for k, v in bs["FIRST_NAME"].items():
            if entname == v:
                keys.append(k)
        if len(
                keys) > 1:  # if we matched on first name and there are a bunch just forget about it
            return None
    # if len(keys) == 0:
...
def get_rels(entry, ents, nums, players, teams, cities):
    ...
    for i, ent in enumerate(ents):
        if ent[3]:  # pronoun
            continue  # for now
        entname = ent[2]
        # assume if a player has a city or team name as his name, they won't
        # use that one (e.g., Orlando Johnson)
...
def append_to_data(
    ...
    for rel in tup[1]:
        ...
        sents.append(sent)
        lens.append(sentlen)
        ent_dists = [j - ent[0] if j < ent[0] else j - ent[1] +
                     1 if j >= ent[1] else 0 for j in range(max_len)]
        entdists.append(ent_dists)
        num_dists = [j - num[0] if j < num[0] else j - num[1] +
                     1 if j >= num[1] else 0 for j in range(max_len)]
...
def save_full_sent_data(
    ...
    if nonedenom > 0:
        ...
        none_idxs = [i for i, labellist in enumerate(
            trlabels) if labellist[0] == labeldict["NONE"]]
        ...
        trsents = [thing for i, thing in enumerate(
            trsents) if i not in ignore_idxs]
        trlens = [thing for i, thing in enumerate(
            trlens) if i not in ignore_idxs]
        trentdists = [thing for i, thing in enumerate(
            trentdists) if i not in ignore_idxs]
        trnumdists = [thing for i, thing in enumerate(
            trnumdists) if i not in ignore_idxs]
        trlabels = [thing for i, thing in enumerate(
            trlabels) if i not in ignore_idxs]
...
def make_pointerfi(
        ...
        content_plan_inp="inter/train_content_plan",
        resolve_prons=False):
    """
    ...
    content_plan = [[record.replace(
        u"￨", "|") for record in x.strip().split()] for x in content_plan]
    ...
    for i, entry in enumerate(trdata):
        content_plan_entry = [
            content_plan_record for content_plan_record in content_plan[i]]
        ...
        inv_vis_players = {pkey: (jj + NUM_PLAYERS)
                    ...
                    if isinstance(idthing, bool):  # city or team
                        ...
                        for k, word in enumerate(tokes[ent_start:ent_end]):
                            ...
                            if word in (
                                    ...
                                    entry["vis_name"],
                                    entry["vis_city"]):
                                src_idx = content_plan_values.index(
                                    word) if word in content_plan_values else -1
                            if src_idx is not None and src_idx != -1:
                                targ_idx = words_so_far + ent_start + k
                                if targ_idx >= len(
                                        entry["summary"]) or entry["summary"][targ_idx] != word:
                                ...
                                if targ_idx is None:
                                    ...
                                    links.append(
                                        (src_idx, targ_idx))  # src_idx, target_idx
                        ...
                        for k, word in enumerate(tokes[num_start:num_end]):
                            ...
                            if idthing:  # home, so look in the home row
                                if entry["home_line"][label] == word:
                                    lookup_record = create_lookup_record(
                                        word, entry["home_line"]["TEAM-NAME"].replace(" ", "_"), label, HOME)
                                    src_idx = content_plan_entry.index(
                                        lookup_record) if lookup_record in content_plan_entry else -1
                            else:
                                if entry["vis_line"][label] == word:
                                    lookup_record = create_lookup_record(
                                        word, entry["vis_line"]["TEAM-NAME"].replace(" ", "_"), label, AWAY)
                                    src_idx = content_plan_entry.index(
                                        lookup_record) if lookup_record in content_plan_entry else -1
                            if src_idx is not None and src_idx != -1:
                        ...
                        if player_row is not None:
                            player_identity = entry["box_score"]['PLAYER_NAME'][idthing].replace(
                                " ", "_")
                            for k, word in enumerate(tokes[ent_start:ent_end]):
                                src_idx = None
                                if word == entry["box_score"]["FIRST_NAME"][idthing]:
                                    lookup_record = create_lookup_record(
                                        word, player_identity, "FIRST_NAME", HOME if is_home else AWAY)
                                    src_idx = content_plan_entry.index(
                                        lookup_record) if lookup_record in content_plan_entry else -1
                                elif word == entry["box_score"]["SECOND_NAME"][idthing]:
                                    lookup_record = create_lookup_record(
                                        word, player_identity, "SECOND_NAME", HOME if is_home else AWAY)
                                    src_idx = content_plan_entry.index(
                                        lookup_record) if lookup_record in content_plan_entry else -1
                                if src_idx is not None and src_idx != -1:
                                    targ_idx = words_so_far + ent_start + k
                            ...
                            for k, word in enumerate(tokes[num_start:num_end]):
                                src_idx = None
                                if word == entry["box_score"][label.split(
                                        '-')[1]][idthing]:
                                    lookup_record = create_lookup_record(
                                        word, player_identity, label.split('-')[1], HOME if is_home else AWAY)
                                    src_idx = content_plan_entry.index(
                                        lookup_record) if lookup_record in content_plan_entry else -1
    ...
    with open(outfi, "w+") as f:
        for links_dict in trlink_dicts:
            ...
            fmtd = [",".join([str(targ_idx)] + [str(thing)
                                                for thing in set(links_dict[targ_idx])]) for targ_idx in targ_idxs]
            f.write("%s\n" % " ".join(fmtd))
...
def save_coref_task_data(outfile, inp_file="full_newnba_prepdata2.json"):
    ...
    for dataset in [data["train"], data["valid"]]:
        ...
        for i, entry in enumerate(dataset):
            ...
            for j in range(1, len(ents)):
                # just get all the words from previous mention till this one
                # starts
    ...
    h5fi["trlens"] = np.array([len(window)
                               for (window, label) in datasets[0]], dtype=int)
    h5fi["trlabels"] = np.array(trlabels, dtype=int)
    h5fi["valwindows"] = np.array(valwindows, dtype=int)
    h5fi["vallens"] = np.array([len(window)
                                for (window, label) in datasets[1]], dtype=int)
    h5fi["vallabels"] = np.array(vallabels, dtype=int)
    #h5fi["vallabelnums"] = np.array(vallabelnums, dtype=int)
    h5fi.close()
...
if args.mode == 'ptrs':
    make_pointerfi(args.output_fi, inp_file=args.input_path,
                   content_plan_inp=args.train_content_plan)
