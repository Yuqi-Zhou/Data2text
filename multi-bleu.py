...
parser = argparse.ArgumentParser(
    description='multi-bleu.py',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('-goldfile', '--goldfile', default='rotowire/gold_valid.txt', help='the file that contains golddata')
parser.add_argument('-referencefile', '--referencefile', default='rotowire/dataset/valid/gen/roto_stage2_cc-beam5_gens.txt', help='the test/valid reference files')
parser.add_argument('-lc', help='Lowercase', default='', action='store_true')
...
def main():
    tokenize_fn = tokenize_lower if opt.lc else tokenize

    ref_file = open(opt.referencefile)
    gold_file = open(opt.goldfile)
    print_multi_bleu(gold_file, ref_file, tokenize_fn, 4)
    ref_file.close()
    gold_file.close()
